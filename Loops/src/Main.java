...
public class Main {
	...
	public static void main(String[] args) throws IOException {
		...
		Employee[] emp_arr = { new Employee("Mark", 123, 1000.0), new Employee("John", 123, 1640.0),
				new Employee("Jhonny", 50, 1025.0), new Employee("Eric", 25, 1026.0),
				new Employee("Anthony", 10, 903.0) };
		System.out.println("Enter dep Number");
		int dep = Integer.parseInt(reader.readLine());
		for (Employee var : emp_arr) {
			if (var.getDepartment_number() == dep) {
				System.out.println(var.getName());
			}
		}

		for (int i = 0; i < emp_arr.length - 1; i++) {
			for (int j = i + 1; j < emp_arr.length; j++) {
				if (emp_arr[i].getSalary() < emp_arr[j].getSalary()) {
